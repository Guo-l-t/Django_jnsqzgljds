...
def update_sy_gzjb_dc(name):
    s = cursor.execute("SELECT * FROM sheet_mubiao_gongzi WHERE `姓名`=%s and `f26`=%s", (name, '3月工资'))
    gzjb_dc = get_sy_gzjb_dc(name)
    gzjb = gzjb_dc[0]
    dc = gzjb_dc[1]
    sql = "UPDATE sheet_mubiao_gongzi SET `工资级别` = %s, `档次` = %s" \
          "WHERE `姓名` = %s and `f26` = %s"
    val = (gzjb, dc, name, '3月工资')
    cursor.execute(sql, val)


def getNames():
    s = cursor.execute("SELECT name FROM sheet_name  ")
    selectResultList = cursor.fetchall()
    for i in range(len(selectResultList)):
        name = selectResultList[i]['name']
        # 查询3月工资信息插入
        # insert_3_gongzi(name)
        # 查询11月工资信息插入
        # insert_11_gongzi(name)
        # update_messages(name)
        update_sy_gzjb_dc(name)
